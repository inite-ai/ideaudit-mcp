...
export function indexSpecs(specs: AnyToolSpec[]): Map<string, AnyToolSpec> {
  const out = new Map<string, AnyToolSpec>();
  for (const s of specs) {
    if (out.has(s.name)) throw new Error(`duplicate tool name: ${s.name}`);
    out.set(s.name, s);
  }
  return out;
}
 ...
 */
export function defineSpec<I, O>(spec: ToolSpec<I, O>): ToolSpec<I, O> {
  return spec;
}
